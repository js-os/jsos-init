...
Launcher.prototype.install = function(moduleName) {
  var install = Promise.promisify(npm.commands.install).bind(this);

  return install([ moduleName ]);
}

Launcher.prototype.uninstall = function(moduleName) {
  var uninstall = Promise.promisify(npm.commands.uninstall).bind(this);

  // First check if we even have the module
  var wd = path.resolve(npm.dir, moduleName);
  ...
  // Otherwise delegate the uninstallation to npm
  return uninstall([ moduleName ]);
}
...
Launcher.prototype.start = function(moduleName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var wd = path.join(npm.dir, moduleName),
        json,
        command,
        args,
        child,
